...
angular.module('sbAdminApp').controller('manageCompanyController', ['$scope', '$location', '$timeout', '$state', 'Notification', 'companyService', 'storageService',
    function ($scope, $location, $timeout, $state, Notification, companyService, storageService) {
        ...
        $scope.hasType = function (enumNumber) {
            return _.any($scope.companyInfo.tradeTypes, function (item) {
                return item.enumValue == enumNumber && item.ticked
            })
        }
        ...
        $scope.$watch('fileContent', function () {
            if ($scope.fileContent) {
                var clientJson = csvJSON($scope.fileContent)
                companyService.addBulkClientToCompany(clientJson).then(function (response) {
                    getCompanyDetail();
        ...
        function csvJSON(csv) {
            ...
            for (var i = 1; i < lines.length; i++) {
                ...
                for (var j = 0; j < headers.length; j++) {
                    obj[headers[j].replace(/ +?/g, '').replace('-', '')] = currentline[j];
                }
